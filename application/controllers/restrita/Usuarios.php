...
class Usuarios extends CI_Controller {
	...
	public function index() {
		...
		public function core($usuario_id = NULL) {
			...
			if(!$usuario_id) {
					...
					$this->form_validation->set_rules('password', 'Senha', 'trim|required|min_length[4]|max_length[200]|callback_valida_usuario');
					...
					if($this->form_validation->run()) {
						...
						$group = array($this->input->post('perfil')); //Sets user para admin ou cliente						
				...
				if(!$usuario = $this->ion_auth->user($usuario_id)->row()) {
					...
					$this->form_validation->set_rules('email', 'E-mail', 'trim|required|min_length[4]|max_length[100]|valid_email|callback_valida_email');
					$this->form_validation->set_rules('username', 'Usuário', 'trim|required|min_length[4]|max_length[50]|callback_valida_usuario');
					$this->form_validation->set_rules('password', 'Senha', 'trim|min_length[4]|max_length[200]|callback_valida_usuario');
					...
					if($this->form_validation->run()) {
						...
						//nao atualiza a senha se nao for passada
						$password = $this->input->post('password');
						if(!$password) {
							unset($data['password']);
						}
	...
	public function valida_email($email) {
		...
		if(!$usuario_id) {
			...
			if($this->core_model->get_by_id('users', array('email' => $email))) {
				$this->form_validation->set_message('valida_email', 'E-mail já existe');
				return false;
			}else{
				return true;
			}
	...
	public function delete($usuario_id = NULL) {
		...
		if(!$usuario_id || !$this->ion_auth->user($usuario_id)->row()) {
			$this->session->set_flashdata('erro', 'Usuário não foi excluído');
			redirect('restrita/usuarios');
		}else{
			if($this->ion_auth->is_admin($usuario_id)) {
				$this->session->set_flashdata('erro', 'Não é perimitido excluir um administrador');
				redirect('restrita/usuarios');
			}
